...
#  New flag to disable async during tests
USE_ASYNC = True

async def fetch_marks(marks_queryset) -> List:
    """
    Asynchronously fetch all student marks.
    """
    if hasattr(marks_queryset, "aall"):
        marks = [m async for m in marks_queryset]
    else:
...
def compute_student_summary(marks_queryset) -> Dict[str, float]:
    """
     If USE_ASYNC is False (during tests), run synchronously to avoid DB locks.
    """
    if not USE_ASYNC:
        gpas = [m.gpa for m in marks_queryset if getattr(m, "gpa", None) is not None]
        return {
            "total_courses": len(marks_queryset),
            "average_gpa": round(average(gpas), 2) if gpas else 0.0
        }

    # Otherwise, use async event loop
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
        loop = None
...
async def generate_report_chart(gpas: List[float]) -> str:
    """
    Generate a GPA distribution chart and return base64 image.
    """
    plt.figure(figsize=(6, 4))
    plt.hist(gpas, bins=5, color='skyblue', edgecolor='black')
    plt.title('GPA Distribution')
